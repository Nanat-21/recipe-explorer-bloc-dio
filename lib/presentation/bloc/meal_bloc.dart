...
class MealBloc extends Bloc<MealEvent, MealState> {
  MealBloc()
    ...
    on<FilterMealsEvent>(_onFilterMeals);
  ...
  /* ================= SAFE HELPER ================= */
  MealLoaded get _currentState => state as MealLoaded;

  /* ================= LOAD ================= */

  void _onLoadMeals(LoadMealsEvent event, Emitter<MealState> emit) {
    emit(MealLoaded(
      meals: List.from(_allMeals),
      favorites: List.from(_favorites),
      mealPlans: List.from(_mealPlans),
  ...
  void _onSearchMeals(SearchMealsEvent event, Emitter<MealState> emit) {
    final filtered = _allMeals
        .where((m) =>
            m.title.toLowerCase().contains(event.query.toLowerCase()))
        .toList();

    emit(_currentState.copyWith(meals: filtered));
  }

  /* ================= FILTER ================= */

  void _onFilterMeals(FilterMealsEvent event, Emitter<MealState> emit) {
    if (event.category == 'All') {
      emit(_currentState.copyWith(meals: List.from(_allMeals)));
      return;
    }
  ...
  void _onToggleFavorite(
      ToggleFavoriteEvent event, Emitter<MealState> emit) {
    final exists = _favorites.any((m) => m.id == event.meal.id);

    if (exists) {
      _favorites.removeWhere((m) => m.id == event.meal.id);
    } else {
  ...
  void _onAddMeal(AddMealEvent event, Emitter<MealState> emit) {
    _allMeals.insert(0, event.meal);
  ...
  void _onDeleteMeal(DeleteMealEvent event, Emitter<MealState> emit) {
    _allMeals.removeWhere((m) => m.id == event.id);
    _favorites.removeWhere((m) => m.id == event.id);
  ...
  void _onAddMealPlan(AddMealPlanEvent event, Emitter<MealState> emit) {
    _mealPlans.add(event.plan);
  ...
  void _onDeleteMealPlan(
      DeleteMealPlanEvent event, Emitter<MealState> emit) {
    _mealPlans.removeAt(event.index);
  ...
  void _onToggleNotification(
      ToggleNotificationEvent event, Emitter<MealState> emit) {
